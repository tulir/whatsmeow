			...
			if action == whatsmeow.ParticipantChangeAdd && item.Error == 403 && item.AddRequest != nil {
				...
				cli.SendMessage(context.TODO(), item.JID, &waProto.Message{
					GroupInviteMessage: &waProto.GroupInviteMessage{
						InviteCode:       waProto.String(item.AddRequest.Code),
						InviteExpiration: waProto.Int64(item.AddRequest.Expiration.Unix()),
						GroupJid:         waProto.String(jid.String()),
						GroupName:        waProto.String("Test group"),
						Caption:          waProto.String("This is a test group"),
					},
				})
