...
func newStarMutation(target, senderJID string, messageID types.MessageID, fromMe string, starred bool) MutationInfo {
	return MutationInfo{
		Index:   []string{IndexStar, target, messageID, fromMe, senderJID},
		Version: 2,
		Value: &waProto.SyncActionValue{
...
// isFromMe should be true if the message was sent by bot.
func BuildStar(target, sender types.JID, messageID types.MessageID, isFromMe, starred bool) PatchInfo {
	fromMe := "0"
	if isFromMe {
		fromMe = "1"
	}
	senderJID, targetJID := sender.ToNonAD().String(), target.ToNonAD().String()
	if sender.User == target.User {
		senderJID = "0"
	}
	return PatchInfo{
		Type: WAPatchRegularHigh,
		Mutations: []MutationInfo{
			newStarMutation(targetJID, senderJID, messageID, fromMe, starred),
		},
	}
}
